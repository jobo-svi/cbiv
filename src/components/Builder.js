...
const PageBuilder = () => {
    ...
    if (activeData.isNewElement) {
      ...
      if (!spacerInsertedRef.current) {
        ...
        if (!updateItems.length) {
          ...
          const nextIndex =
            overData.rowIndex > -1 ? overData.rowIndex : updateItems.length;
        ...
        const nextIndex =
          overData.rowIndex > -1 ? overData.rowIndex : updateItems.length - 1;

        if (nextIndex === spacerIndex) {
          return;
        }

        updateItems = arrayMove(updateItems, spacerIndex, overData.rowIndex);
        setItems(updateItems);
      }
    }
  };
  ...
  const handleDragEnd = (e) => {
    ...
    if (nextField) {
      ...
      setItems(
        moveElement(updateItems, nextField, overData.rowIndex, eleToCombine)
      );
    }
              ...
              >
                {items.length > 0 &&
                  items.map((row, rowIndex) => {
                    return (
                      <div className="grid-row" key={row.id} id={row.id}>
                        {row.columns.map((col, colIndex) => {
                          return (
                            <div className="grid-column" key={col.id}>
                              <SortableItem
                                key={col.id}
                                id={col.id}
                                rowIndex={rowIndex}
